...
#for(th in c(0.1)){
	...
	for(i in as.character(unique(mag$sample))){ #i=as.character(unique(mag $sample))[5]
		...
		sq0 <- proc.time()[3]
		sim <- mean(1-vegdist(sub, method='horn'))
		nodfcq0 <- NODFc(sub, quality = 0)[[1]]
		eq0 <- proc.time()[3]
		cat(sprintf('q0 day %s done: elapsed time %0.2f sec\n',i,  eq0 - sq0))

		## =========================== ## 
		 
		statniche <- c(sim, nodfcq0)
		gc();   gc()
		
		etmp <- proc.time()[3]
		cat(sprintf('Day %s done: elapsed time %0.2f sec\n\n',i,  etmp - stmp))
		
		statsum <- rbind(statsum, statniche)
	}

#}

saveRDS(statsum, sprintf('%s/04_06_randamization_th%s.rds', dir$rdsdir, th) )
e <- Sys.time()
